...
def commit_milestones(milestones: List, state: PlanState):
    ...
    for m in milestones:
        milestone_obj = Milestone(
            ...
            goal_id=state["structured_data"]["goal"]["id"],
            statement=m["statement"],
        )

        for t in m.get("tracker", []):
            config = get_metric(t)
...
def update_state_on_response(state: PlanState, response: BaseMessage):
    ...
    milestone_details = response_json.get("milestone_details")
...
def run_milestone_formulator(state: PlanState):
    ...
    try:
        ...
        logger.error(f"LLM Error in Milestone Formulator: {e}")
        return state
